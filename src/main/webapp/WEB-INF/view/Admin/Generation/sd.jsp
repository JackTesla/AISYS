...
<%--        seed: 开始<input id="seeds-start" type="text" name="cmd" value="" placeholder="" >--%>
    ...
    <script type="text/javascript">
        ...
        function updatePage(data) {
            ...
            for(var url in data.data) {
                if(data.data[url] !== undefined) {
                    outputs += "<div>"+"<p>" +
                        "<img class='sd-pics' src=/uploads/sd/" + data.data[url] + " />" +
                        "<form class='picdelForm' name='picdelForm' method='post' action='#'" + ">" +
                        "<input type=hidden class='del_url' value=" + data.data[url] + " />" +
                        "<a class='picDownload' href=/uploads/sd/"+ data.data[url]+" download>下载</a>" +
        ...
        function picDel(element) {
            ...
            $.ajax({
                async: false,//同步，待请求完毕后再执行后面的代码
                type: "POST",
                url: '/admin/generation/sdPicDelete',
                contentType: "application/x-www-form-urlencoded; charset=utf-8",
                data: {del_url:delUrl},
                dataType: "json",
                success: function (data) {
