...
//        for(String dstDisk:dstDisks) new File(dstDisk+dstDir).mkdirs();
        ...
        replyString=scanner.nextLine();
        if(replyString.equals("Y") || replyString.equals("y")) recursive=true;
    ...
    public static void main(String[] args) throws InterruptedException {
        LinuxDirFileCopy fileAndDirCopy=new LinuxDirFileCopy();
...
//        System.out.println("allFileSet size:"+allFileSet.size());
        ...
        for (File file : allFiles) {
            if(file.isFile() ){
                if ((allFileSet.add(file.toString())) && (!file.toString().equals(finishedLogPath))) {
                    System.out.println(file);
                    taskFileList.add(file.toString());
//                allFileSet.add(file.toString());
                }
            } else fileLinkedList.add(file);
        }
        if(recursive) {
            while (fileLinkedList.isEmpty() == false) {
                File file = fileLinkedList.pop();
                if (file.isFile()) {
                    if ((allFileSet.add(file.toString())) && (!file.toString().equals(finishedLogPath))) {
                        System.out.println(file);
                        taskFileList.add(file.toString());
//                allFileSet.add(file.toString());
                    ...
                    for(File afile:tmpFiles) fileLinkedList.add(afile);
                }
            }
        }
//        System.out.println("allFileSet size:"+allFileSet.size());
